...
class Node:
    ...
    def __init__(self,init_x,init_y,poss_rotate,leg):
        self.leg = leg
...
def r_left():
    if lander.rcsvel >= -25: lander.rcsvel += rcs
...
def mthrottle():
    ...
    for node in craft:
        node.xvel += (-1*(thr[0])/100)*main_eng
        node.yvel += (-1*(thr[1])/100)*main_eng
...
def check_collision():
    global crashed,landed,l_text,ground,lt
    #Checks if rocket is out of bounds
    #if lander.x >= 500 or lander.x <= 0 or lander.y <= 0 or landed: restart()
    for node in craft:
        ground_y = gen_precise_collider(ground,500,node.x)
        if node.y >= ground_y:
            if node.leg:
                leg = node
            else: pass
            #else: landed, crashed = True,True
        elif lt == node: lt = None
    if lander.xvel == 0 and lander.yvel == 0 and lander.rcsvel == 0: landed = True

def bounce_ang():
    rev = False
    ground_ang = math.degrees(math.atan2((ground[1][1]-ground[0][1]),(ground[1][0]-ground[0][0])))
    leg_ang = math.degrees(math.atan2(leg.yvel,leg.xvel))
    if ground_ang < 0: ground_ang += 180
    if leg_ang < 0: leg_ang += 180
    if ground_ang < leg_ang:
        ground_ang += 90
        leg_ang -= 90
        rev = True
    inv_ground_ang = (ground_ang + 180) % 360
    if inv_ground_ang < ground_ang:
        t = ground_ang
        ground_ang = inv_ground_ang
        inv_ground_ang = t
    a = ground_ang - leg_ang
    b = inv_ground_ang + a
    if rev: b -= 90
    return b % 360
...
def on_bounce():
    ang = bounce_ang()
    print(ang)
    slope = math.tan(ang)*100
    sum_slope = slope+100
    sum_spd = leg.xvel+leg.yvel
    return (100/sum_slope) * sum_spd,(slope/sum_slope) * sum_spd
...
l_text = None
disp_text = None
lander = Node(250,50,True,False)
nose = Node(250,30,False,False)
l1 = Node(240,60,True,True)
l2 = Node(260,60,True,True)
landed = False
crashed = False
sas_flag = False
ground = create_ground(8,50,200,500)
